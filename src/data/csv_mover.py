...
class CSVMover:
    ...
    def import_csv(parent_directory: str, file_name: str) -> pd.DataFrame:
        ...
        full_file_path = os.path.join("data", parent_directory, f"{file_name}.csv")
    ...
    def export_csv(df: pd.DataFrame, file_name: str) -> None:
        """
        Exports a DataFrame to a CSV file in the data/processed directory of the repository.
        ...
        Raises:
            TypeError: If df or file_name is not of the expected type.
        ...
        path_to_file = f"data/processed/{file_name}.csv"
        try:
